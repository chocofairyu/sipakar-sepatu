...
.admin-hero {
    ...
    box-shadow: 0 14px 32px rgba(18,60,58,0.16);
}
.admin-hero h1 {
    color: #FFFFFF !important;
    margin: 0 !important;
...
.admin-kpi {
    ...
    border-top: 4px solid var(--kpi-color, #1F7A68);
    border-radius: 8px;
    padding: 1rem 1.05rem;
    box-shadow: 0 8px 22px rgba(18,60,58,0.06);
}
.admin-kpi-label {
    color: #71827F;
    font-size: 14px;
...
<div class="admin-hero">
  <div>
    ...
    <p>Pantau aktivitas konsultasi dan kualitas rekomendasi sistem.</p>
  </div>
  <div class="admin-status">Sistem aktif</div>
</div>
""", unsafe_allow_html=True)
    ...
    if riwayat:
        ...
        df_r["CF Akhir"] = pd.to_numeric(df_r["CF Akhir"], errors="coerce").fillna(0)
        df_r["Status"] = pd.cut(
            df_r["CF Akhir"],
            bins=[-0.01, 0.39, 0.59, 1.0],
            labels=["Perlu perhatian", "Cukup yakin", "Sangat yakin"],
        )
        ...
        avg_cf = df_r["CF Akhir"].mean() * 100
        today_count = int((df_r["Tanggal"].dt.date == pd.Timestamp.today().date()).sum())

        st.markdown(f"""
<div class="admin-kpi-grid">
  <div class="admin-kpi" style="--kpi-color:#1F7A68">
    <div class="admin-kpi-label">Total konsultasi</div>
    <div class="admin-kpi-value">{stat['total']}</div>
    <div class="admin-kpi-note">{today_count} konsultasi hari ini</div>
  </div>
  <div class="admin-kpi" style="--kpi-color:#2878B5">
    <div class="admin-kpi-label">Rata-rata keyakinan</div>
    <div class="admin-kpi-value">{avg_cf:.1f}%</div>
    <div class="admin-kpi-note">Dari 100 data terbaru</div>
  </div>
  <div class="admin-kpi" style="--kpi-color:#E7A93B">
    <div class="admin-kpi-label">Metode terpopuler</div>
    <div class="admin-kpi-value">{top_metode}</div>
    <div class="admin-kpi-note">Paling sering direkomendasikan</div>
  </div>
  <div class="admin-kpi" style="--kpi-color:#C45A4A">
    <div class="admin-kpi-label">Bahan terbanyak</div>
    <div class="admin-kpi-value">{top_bahan}</div>
    <div class="admin-kpi-note">Paling sering dikonsultasikan</div>
  </div>
</div>
""", unsafe_allow_html=True)

        tab_overview, tab_data = st.tabs(["Overview", "Data konsultasi"])

        with tab_overview:
            chart_col, distribution_col = st.columns([3, 2], gap="large")
            with chart_col:
                st.markdown('<div class="admin-section-title">Tren 14 hari terakhir</div>', unsafe_allow_html=True)
                daily = (
                    df_r.set_index("Tanggal")
                    .resample("D")["ID"]
                    .count()
                    .tail(14)
                    .rename("Konsultasi")
                )
                st.line_chart(daily, color="#1F7A68", height=290)

            with distribution_col:
                st.markdown('<div class="admin-section-title">Distribusi keyakinan</div>', unsafe_allow_html=True)
                status_counts = (
                    df_r["Status"].value_counts(sort=False)
                    .rename_axis("Status")
                    .reset_index(name="Jumlah")
                )
                st.bar_chart(status_counts.set_index("Status"), color="#E7A93B", height=290)

            recent_col, material_col = st.columns([3, 2], gap="large")
            with recent_col:
                st.markdown('<div class="admin-section-title">Aktivitas terbaru</div>', unsafe_allow_html=True)
                recent = df_r.sort_values("Tanggal", ascending=False).head(6).copy()
                recent["Waktu"] = recent["Tanggal"].dt.strftime("%d %b, %H:%M")
                recent["Keyakinan"] = (recent["CF Akhir"] * 100).round(1)
                st.dataframe(
                    recent[["Nama", "Bahan", "Metode", "Keyakinan", "Waktu"]],
                    use_container_width=True,
                    hide_index=True,
                    column_config={
                        "Keyakinan": st.column_config.ProgressColumn(
                            "Keyakinan", format="%.1f%%", min_value=0, max_value=100
                        )
                    },
                    height=250,
                )
            ...
            with material_col:
                st.markdown('<div class="admin-section-title">Bahan paling sering</div>', unsafe_allow_html=True)
                material_counts = df_r["Bahan"].value_counts().head(6).rename("Jumlah")
                st.bar_chart(material_counts, horizontal=True, color="#2878B5", height=250)

        with tab_data:
            st.markdown('<div class="admin-section-title">Filter data</div>', unsafe_allow_html=True)
            min_date = df_r["Tanggal"].min().date()
            max_date = df_r["Tanggal"].max().date()
            f1, f2, f3, f4 = st.columns([2, 1.3, 1.3, 1.5])
            with f1:
                cari_nama = st.text_input(
                    "Cari pengguna", placeholder="Nama pengguna...", key="filter_riwayat_nama"
                )
            with f2:
                filter_bahan_riwayat = st.selectbox(
                    "Bahan", ["Semua"] + sorted(df_r["Bahan"].dropna().unique().tolist()),
                    key="filter_riwayat_bahan",
                )
            with f3:
                filter_metode_riwayat = st.selectbox(
                    "Metode", ["Semua"] + sorted(df_r["Metode"].dropna().unique().tolist()),
                    key="filter_riwayat_metode",
                )
            with f4:
                rentang_tanggal = st.date_input(
                    "Rentang tanggal", value=(min_date, max_date), min_value=min_date,
                    max_value=max_date, key="filter_riwayat_tanggal",
                )

            df_filtered = df_r.copy()
            if cari_nama:
                df_filtered = df_filtered[
                    df_filtered["Nama"].fillna("").str.contains(cari_nama, case=False, regex=False)
                ]
            if filter_bahan_riwayat != "Semua":
                df_filtered = df_filtered[df_filtered["Bahan"] == filter_bahan_riwayat]
            if filter_metode_riwayat != "Semua":
                df_filtered = df_filtered[df_filtered["Metode"] == filter_metode_riwayat]
            ...
            result_col, export_col = st.columns([4, 1])
            with result_col:
                st.caption(f"Menampilkan {len(df_filtered)} dari {len(df_r)} konsultasi")
            with export_col:
                csv_data = df_filtered.to_csv(index=False).encode("utf-8")
                st.download_button(
                    "Unduh CSV", data=csv_data, file_name="riwayat_konsultasi.csv",
                    mime="text/csv", use_container_width=True,
                )

            table_df = df_filtered.sort_values("Tanggal", ascending=False).copy()
            table_df["Keyakinan"] = (table_df["CF Akhir"] * 100).round(1)
            table_df["Tanggal"] = table_df["Tanggal"].dt.strftime("%d/%m/%Y %H:%M")
            st.dataframe(
                table_df[["ID", "Nama", "Bahan", "Noda", "Metode", "Status", "Keyakinan", "Tanggal"]],
                use_container_width=True,
                hide_index=True,
                column_config={
                    "Keyakinan": st.column_config.ProgressColumn(
...
<div class="admin-kpi-grid">
  <div class="admin-kpi" style="--kpi-color:#1F7A68">
    ...
    <div class="admin-kpi-note">Belum ada aktivitas</div>
  </div>
</div>
<div class="admin-empty">Belum ada riwayat konsultasi untuk ditampilkan.</div>
""", unsafe_allow_html=True)
